...
import com.tecdatum.Tracking.School.newactivities.*
...
import kotlinx.android.synthetic.main.fragment_livetracking.*
...
class LivetrackingFragment : Fragment(), OnMapReadyCallback {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        viewd = inflater.inflate(R.layout.fragment_livetracking, container, false)
        map = viewd!!.findViewById(R.id.list_item_map_view_mapview_livetracking)
        if (map != null) {
            map!!.onCreate(null)
            map!!.onResume()
        ...
        if (bb != null) {
           ...
           /* cb_pointsm_livetracking.setOnCheckedChangeListener(CompoundButton.OnCheckedChangeListener { compoundButton, b ->
                ...
                timer.schedule(object : TimerTask() {
                    override fun run() {
                        ...
                        if ((connec != null && (((connec.getNetworkInfo(ConnectivityManager.TYPE_WIFI).getState() === NetworkInfo.State.CONNECTED) || (connec.getNetworkInfo(ConnectivityManager.TYPE_MOBILE).getState() === NetworkInfo.State.CONNECTED))))) {
                            service()
                                try {
                                    getDirection(lat, lng, 17.4353669,78.44083076)
//                                    getDirection_distance(lat, lng, 17.4353669!!.toDouble(), 78.44083076!!.toDouble())
    ...
    private fun isGooglePlayServicesAvailable(): Boolean {
        ...
        if (ConnectionResult.SUCCESS == status) return true else {
            if (googleApiAvailability.isUserResolvableError(status)) Toast.makeText(activity!!, "Please Install google play services to use this application", Toast.LENGTH_LONG).show()
        }
        return false
    }

    fun service() {
        val bb: SharedPreferences = context!!.getSharedPreferences(SplashScreen.MY_PREFS_NAME, Context.MODE_PRIVATE)
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<String?>, grantResults: IntArray) {
        ...
        if (requestCode == MY_PERMISSIONS_REQUEST_ACCESS_FINE_LOCATION) {
            if (grantResults[0] == PackageManager.PERMISSION_DENIED) Toast.makeText(activity!!, "Permission denied by uses", Toast.LENGTH_SHORT).show() else if (grantResults[0] == PackageManager.PERMISSION_GRANTED) startCurrentLocationUpdates()
        }
    }
    ...
    fun drawPath(result: String?) {
        ...
        currentLocationMarker_Destination = googleMap!!.addMarker(MarkerOptions()
                .position(LatLng(17.4353669,78.44083076)).title(Title)
                .draggable(false)
